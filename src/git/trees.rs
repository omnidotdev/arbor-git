...
impl TreeService {
    ...
    pub fn get_tree(
        ...
        entries.sort_by(|a, b| {
            match (
                a.entry_type == EntryType::Tree,
                b.entry_type == EntryType::Tree,
            ) {
    ...
    /// Get blob content
    #[instrument(skip(self))]
    pub fn get_blob(&self, owner: &str, name: &str, oid: &str) -> Result<Vec<u8>> {
        let repo = open_repo_by_name(&self.config, owner, name)?;
        ...
        let blob = repo.find_blob(id).map_err(|_| GitError::ObjectNotFound {
            oid: oid.to_string(),
        })?;
    ...
    #[instrument(skip(self))]
    pub fn get_blob_info(&self, owner: &str, name: &str, oid: &str) -> Result<BlobInfo> {
        let repo = open_repo_by_name(&self.config, owner, name)?;
    ...
    fn collect_tree_entries(
        ...
        for entry in tree.iter() {
            ...
            if entry_type == EntryType::Tree
                ...
                && let Ok(subtree) = repo.find_tree(entry.object_id())
            {
