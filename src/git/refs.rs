...
impl RefService {
    ...
    pub fn list_refs(
        ...
        let refs = repo.references().map_err(|e| GitError::Gix(e.to_string()))?;
        ...
        let iter: Box<dyn Iterator<Item = _>> = match filter_type {
            Some(RefType::Branch) => {
                Box::new(refs.local_branches().map_err(|e| GitError::Gix(e.to_string()))?)
            }
            Some(RefType::Tag) => {
                Box::new(refs.tags().map_err(|e| GitError::Gix(e.to_string()))?)
            }
            Some(RefType::Remote) => {
                Box::new(refs.remote_branches().map_err(|e| GitError::Gix(e.to_string()))?)
            }
        ...
        for reference in iter {
            ...
            if let Some(pat) = pattern {
                if !glob_match(&short_name, pat) && !glob_match(&full_name, pat) {
                    continue;
                }
            }
    ...
    #[instrument(skip(self))]
    pub fn resolve_ref(&self, owner: &str, name: &str, reference: &str) -> Result<(String, String)> {
        let repo = open_repo_by_name(&self.config, owner, name)?;
    ...
    /// Delete a branch
    #[instrument(skip(self))]
    pub fn delete_branch(&self, owner: &str, name: &str, branch_name: &str, force: bool) -> Result<bool> {
        let repo = open_repo_by_name(&self.config, owner, name)?;
        ...
        if !force {
            if let Ok(Some(head_ref)) = repo.head_ref() {
                if head_ref.name().as_bstr() == ref_name.as_bytes() {
...
mod tests {
    ...
    fn test_determine_ref_type() {
        ...
        assert_eq!(determine_ref_type("refs/remotes/origin/main"), RefType::Remote);
    }
}
