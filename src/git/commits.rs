...
impl CommitService {
    ...
    pub fn get_commit(&self, owner: &str, name: &str, oid: &str) -> Result<CommitInfo> {
        ...
        let commit = repo.find_commit(id).map_err(|_| GitError::ObjectNotFound {
            oid: oid.to_string(),
        })?;
    ...
    pub fn find_merge_base(
        ...
        let id1 =
            gix::ObjectId::from_hex(oid1.as_bytes()).map_err(|_| GitError::ObjectNotFound {
                oid: oid1.to_string(),
            })?;

        let id2 =
            gix::ObjectId::from_hex(oid2.as_bytes()).map_err(|_| GitError::ObjectNotFound {
                oid: oid2.to_string(),
            })?;
...
fn commit_to_info(commit: &gix::Commit) -> CommitInfo {
    ...
    CommitInfo {
        ...
        author: GitActor {
            ...
            offset_minutes: author_time.offset,
        },
        committer: GitActor {
