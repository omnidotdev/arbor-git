...
impl OperationsService {
    ...
    pub fn merge(
        ...
        let base_tree = base_commit.tree().map_err(|e| GitError::Gix(e.to_string()))?;
        let head_tree = head_commit.tree().map_err(|e| GitError::Gix(e.to_string()))?;
        let ancestor_tree = ancestor_commit.tree().map_err(|e| GitError::Gix(e.to_string()))?;

        // Perform tree merge
        let merge_result = self.merge_trees(
            &repo,
            &ancestor_tree,
            &base_tree,
            &head_tree,
    ...
    pub fn cherry_pick(
        ...
        let parent_tree = parent_commit.tree().map_err(|e| GitError::Gix(e.to_string()))?;
        let commit_tree = commit.tree().map_err(|e| GitError::Gix(e.to_string()))?;
        let onto_tree = onto_commit.tree().map_err(|e| GitError::Gix(e.to_string()))?;
        ...
        ancestor
            ...
            .for_each_to_obtain_tree(ours, |change| {
                ...
                let info = match change {
                    ...
                    Change::Modification { location, previous_id, id, .. } => ChangeInfo {
                        path: location.to_string(),
